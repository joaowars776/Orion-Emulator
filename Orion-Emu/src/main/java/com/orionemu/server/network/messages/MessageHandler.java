package com.orionemu.server.network.messages;

import com.orionemu.server.boot.Orion;
import com.orionemu.server.config.Configuration;
import com.orionemu.server.network.messages.incoming.Event;
import com.orionemu.server.network.messages.incoming.catalog.*;
import com.orionemu.server.network.messages.incoming.catalog.ads.PromoteRoomMessageEvent;
import com.orionemu.server.network.messages.incoming.catalog.ads.PromotionUpdateMessageEvent;
import com.orionemu.server.network.messages.incoming.catalog.data.GetGiftWrappingConfigurationMessageEvent;
import com.orionemu.server.network.messages.incoming.catalog.groups.BuyGroupDialogMessageEvent;
import com.orionemu.server.network.messages.incoming.catalog.groups.BuyGroupMessageEvent;
import com.orionemu.server.network.messages.incoming.catalog.groups.GroupFurnitureCatalogMessageEvent;
import com.orionemu.server.network.messages.incoming.catalog.pets.PetRacesMessageEvent;
import com.orionemu.server.network.messages.incoming.catalog.pets.ValidatePetNameMessageEvent;
import com.orionemu.server.network.messages.incoming.gamecenter.*;
import com.orionemu.server.network.messages.incoming.gamecenter.snowwar.*;
import com.orionemu.server.network.messages.incoming.group.*;
import com.orionemu.server.network.messages.incoming.group.favourite.ClearFavouriteGroupMessageEvent;
import com.orionemu.server.network.messages.incoming.group.favourite.SetFavouriteGroupMessageEvent;
import com.orionemu.server.network.messages.incoming.group.forum.data.ForumDataMessageEvent;
import com.orionemu.server.network.messages.incoming.group.forum.data.GetForumsMessageEvent;
import com.orionemu.server.network.messages.incoming.group.forum.settings.SaveForumSettingsMessageEvent;
import com.orionemu.server.network.messages.incoming.group.forum.threads.*;
import com.orionemu.server.network.messages.incoming.group.settings.*;
import com.orionemu.server.network.messages.incoming.handshake.*;
import com.orionemu.server.network.messages.incoming.help.GetSanctionStatusEvent;
import com.orionemu.server.network.messages.incoming.help.HelpTicketMessageEvent;
import com.orionemu.server.network.messages.incoming.help.InitBullyingHelpToolMessageEvent;
import com.orionemu.server.network.messages.incoming.help.InitSexualHelpToolMessageEvent;
import com.orionemu.server.network.messages.incoming.help.guides.GuideHandleHelpRequestMessageEvent;
import com.orionemu.server.network.messages.incoming.help.guides.OpenGuideToolMessageEvent;
import com.orionemu.server.network.messages.incoming.help.guides.RequestGuideAssistanceMessageEvent;
import com.orionemu.server.network.messages.incoming.landing.LandingLoadWidgetMessageEvent;
import com.orionemu.server.network.messages.incoming.landing.RefreshPromoArticlesMessageEvent;
import com.orionemu.server.network.messages.incoming.messenger.*;
import com.orionemu.server.network.messages.incoming.moderation.*;
import com.orionemu.server.network.messages.incoming.moderation.tickets.ModToolCloseIssueMessageEvent;
import com.orionemu.server.network.messages.incoming.moderation.tickets.ModToolPickTicketMessageEvent;
import com.orionemu.server.network.messages.incoming.moderation.tickets.ModToolReleaseIssueMessageEvent;
import com.orionemu.server.network.messages.incoming.moderation.tickets.ModToolTicketChatlogMessageEvent;
import com.orionemu.server.network.messages.incoming.music.SongDataMessageEvent;
import com.orionemu.server.network.messages.incoming.music.SongIdMessageEvent;
import com.orionemu.server.network.messages.incoming.music.playlist.PlaylistMessageEvent;
import com.orionemu.server.network.messages.incoming.music.playlist.PlaylistRemoveMessageEvent;
import com.orionemu.server.network.messages.incoming.navigator.*;
import com.orionemu.server.network.messages.incoming.performance.EventLogMessageEvent;
import com.orionemu.server.network.messages.incoming.performance.RequestLatencyTestMessageEvent;
import com.orionemu.server.network.messages.incoming.polls.GetPollMessageEvent;
import com.orionemu.server.network.messages.incoming.polls.SubmitPollAnswerMessageEvent;
import com.orionemu.server.network.messages.incoming.quests.CancelQuestMessageEvent;
import com.orionemu.server.network.messages.incoming.quests.OpenQuestsMessageEvent;
import com.orionemu.server.network.messages.incoming.quests.StartQuestMessageEvent;
import com.orionemu.server.network.messages.incoming.room.access.AnswerDoorbellMessageEvent;
import com.orionemu.server.network.messages.incoming.room.access.LoadRoomByDoorBellMessageEvent;
import com.orionemu.server.network.messages.incoming.room.action.*;
import com.orionemu.server.network.messages.incoming.room.bots.BotConfigMessageEvent;
import com.orionemu.server.network.messages.incoming.room.bots.ModifyBotMessageEvent;
import com.orionemu.server.network.messages.incoming.room.bots.PlaceBotMessageEvent;
import com.orionemu.server.network.messages.incoming.room.bots.RemoveBotMessageEvent;
import com.orionemu.server.network.messages.incoming.room.engine.*;
import com.orionemu.server.network.messages.incoming.room.filter.EditWordFilterMessageEvent;
import com.orionemu.server.network.messages.incoming.room.filter.WordFilterListMessageEvent;
import com.orionemu.server.network.messages.incoming.room.floor.SaveFloorMessageEvent;
import com.orionemu.server.network.messages.incoming.room.item.*;
import com.orionemu.server.network.messages.incoming.room.item.gifts.OpenGiftMessageEvent;
import com.orionemu.server.network.messages.incoming.room.item.lovelock.ConfirmLoveLockMessageEvent;
import com.orionemu.server.network.messages.incoming.room.item.mannequins.SaveMannequinFigureMessageEvent;
import com.orionemu.server.network.messages.incoming.room.item.mannequins.SaveMannequinMessageEvent;
import com.orionemu.server.network.messages.incoming.room.item.stickies.DeletePostItMessageEvent;
import com.orionemu.server.network.messages.incoming.room.item.stickies.OpenPostItMessageEvent;
import com.orionemu.server.network.messages.incoming.room.item.stickies.PlacePostItMessageEvent;
import com.orionemu.server.network.messages.incoming.room.item.stickies.SavePostItMessageEvent;
import com.orionemu.server.network.messages.incoming.room.item.wired.SaveWiredDataMessageEvent;
import com.orionemu.server.network.messages.incoming.room.item.wired.UpdateSnapshotsMessageEvent;
import com.orionemu.server.network.messages.incoming.room.moderation.*;
import com.orionemu.server.network.messages.incoming.room.pets.*;
import com.orionemu.server.network.messages.incoming.room.pets.horse.ApplyHorseEffectMessageEvent;
import com.orionemu.server.network.messages.incoming.room.pets.horse.ModifyWhoCanRideHorseMessageEvent;
import com.orionemu.server.network.messages.incoming.room.pets.horse.RemoveHorseSaddleMessageEvent;
import com.orionemu.server.network.messages.incoming.room.pets.horse.RideHorseMessageEvent;
import com.orionemu.server.network.messages.incoming.room.settings.*;
import com.orionemu.server.network.messages.incoming.room.trading.*;
import com.orionemu.server.network.messages.incoming.talenttrack.GetTalentTrackEvnet;
import com.orionemu.server.network.messages.incoming.user.achievements.AchievementsListMessageEvent;
import com.orionemu.server.network.messages.incoming.user.club.ClubStatusMessageEvent;
import com.orionemu.server.network.messages.incoming.user.details.*;
import com.orionemu.server.network.messages.incoming.user.inventory.*;
import com.orionemu.server.network.messages.incoming.user.nuxInstructionEvent;
import com.orionemu.server.network.messages.incoming.user.profile.*;
import com.orionemu.server.network.messages.incoming.user.wardrobe.ChangeLooksMessageEvent;
import com.orionemu.server.network.messages.incoming.user.wardrobe.SaveWardrobeMessageEvent;
import com.orionemu.server.network.messages.incoming.user.wardrobe.WardrobeMessageEvent;
import com.orionemu.server.network.messages.incoming.user.youtube.LoadPlaylistMessageEvent;
import com.orionemu.server.network.messages.incoming.user.youtube.NextVideoMessageEvent;
import com.orionemu.server.network.messages.incoming.user.youtube.PlayVideoMessageEvent;
import com.orionemu.server.network.messages.outgoing.group.DeleteGroupMessageEvent;
import com.orionemu.server.network.messages.protocol.messages.MessageEvent;
import com.orionemu.server.network.messages.types.tasks.MessageEventTask;
import com.orionemu.server.network.sessions.Session;
import com.orionemu.server.network.messages.protocol.headers.Events;
...
public final class MessageHandler {
    ...
    public MessageHandler() {
        this.asyncEventExecution = Boolean.parseBoolean((String) Configuration.currentConfig().getOrDefault("orion.network.alternativePacketHandling.enabled", "false"));
        //        this.eventExecutor = asyncEventExecution ? Executors.newFixedThreadPool(Integer.parseInt((String) Configuration.currentConfig().getOrDefault("orion.network.alternativePacketHandling.threads", "8"))) : null;

        if (this.asyncEventExecution) {
            switch ((String) Configuration.currentConfig().getOrDefault("orion.network.alternativePacketHandling.type", "threadpool")) {
                default:
                    log.info("Using thread-pool event executor");
                    this.eventExecutor = new ThreadPoolExecutor(Integer.parseInt((String) Configuration.currentConfig().getOrDefault("orion.network.alternativePacketHandling.coreSize", "8")), // core size
                            Integer.parseInt((String) Configuration.currentConfig().getOrDefault("orion.network.alternativePacketHandling.maxSize", "32")), // max size
                            10 * 60, // idle timeout
                ...
                case "forkjoin":
                    ...
                    this.eventExecutor = new ForkJoinPool(Integer.parseInt((String) Configuration.currentConfig().getOrDefault("orion.network.alternativePacketHandling.coreSize", 16)), ForkJoinPool.defaultForkJoinWorkerThreadFactory, null, true);
                    break;
            }
        } else {
    ...
    public void registerHelpTool() {
        this.getMessages().put(Events.InitializeSexualHelpToolEvent, new InitSexualHelpToolMessageEvent());
        this.getMessages().put(Events.InitializeBullyingHelpToolEvent, new InitBullyingHelpToolMessageEvent());
    ...
    public void registerUser() {
        ...
        //här fattar några som ska läggas in
    ...
    public void registerRoom() {
        ...
        this.getMessages().put(Events.WisperMessageEvent, new WhisperMessageEvent());
    ...
    public void registerMusic() {
        ...
        //this.getMessages().put(Events.PlaylistAddMessageEvent, new PlaylistAddMessageEvent()); TODO: Fix jukeboxes
    ...
    public void registerNux() {
        this.getMessages().put(Events.NuxInstructionMessageEvent, new nuxInstructionEvent());
    }

    public void registerGameCenter() {
        this.getMessages().put(Events.InitializeGamecenterEvent, new InitializeGameCenterEvent());
        this.getMessages().put(Events.GameCenterRequestGamesEvent, new GetGameListingEvent());
        this.getMessages().put(Events.GetPlayableGamesMessageEvent, new GetPlayableGamesEvent());
        this.getMessages().put(Events.GameCenterJoinGameEvent, new JoinGameEvent());
    }
    ...
    private void registerSnowstorm() {
        ...
        this.getMessages().put(Events.ReuquestFullStatusUpdateParser, new RequestFullStatusUpdateParser());
        this.getMessages().put(Events.ExitGameParser, new ExitGameParser());
        this.getMessages().put(Events.LeaveGameParser, new LeaveGameParser());
        ...
        this.getMessages().put(Events.MakeSnowballParser, new MakeSnowballParser());
    }

    private void registerTalentTrack() {
        this.getMessages().put(Events.GetTalentTrackMessageEvent, new GetTalentTrackEvnet());
    }
