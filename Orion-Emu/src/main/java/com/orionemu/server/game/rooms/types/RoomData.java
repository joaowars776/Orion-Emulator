...
import com.orionemu.api.game.rooms.IRoomData;
import com.orionemu.api.game.rooms.RoomCategory;
...
public class RoomData implements IRoomData {
    ...
    private long lastReferenced = Orion.getTime();
    ...
    public void save() {
        ...
        if (OrionSettings.roomEncryptPasswords) {
            if (!this.password.equals(this.originalPassword)) {
                this.password = BCrypt.hashpw(this.password, BCrypt.gensalt(OrionSettings.roomPasswordEncryptionRounds));
            }
        }
