package com.orionemu.server.game.rooms.objects.items;
...
import com.orionemu.server.game.rooms.objects.items.types.DefaultWallItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.*;
import com.orionemu.server.game.rooms.objects.items.types.floor.banzai.*;
import com.orionemu.server.game.rooms.objects.items.types.floor.boutique.MannequinFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.football.*;
import com.orionemu.server.game.rooms.objects.items.types.floor.groups.GroupFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.groups.GroupGateFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.hollywood.HaloTileFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.pet.HorseWalkOnItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.pet.PetToyFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.pet.horse.HorseJumpFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.snowboarding.SnowboardJumpFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.snowboarding.SnowboardSlopeFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.summer.SummerShowerFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.totem.TotemBodyFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.totem.TotemHeadFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.totem.TotemPlanetFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.actions.*;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.addons.*;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.conditions.negative.*;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.conditions.positive.*;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.highscore.HighscoreClassicFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.highscore.JackpotClassicFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.triggers.*;
...
import com.orionemu.server.game.rooms.objects.items.types.wall.WheelWallItem;
...
public class RoomItemFactory {
    ...
    static {
        itemDefinitionMap = new HashMap<String, Class<? extends RoomItemFloor>>() {{
            ...
            put("vendingmachine", IcecreamMachineFloorItem.class);
            put("icecream", VendingMachineFloorItem.class);
            put("case", CaseFloorItem.class);
            ...
            put("horse_walk_on", HorseWalkOnItem.class);
            ...
            put("wf_act_orion", WiredActionOrion.class);//new
            ...
            put("highscore_classic", HighscoreClassicFloorItem.class); //sec
            put("jackdpot_classic", JackpotClassicFloorItem.class); //aj aj nu kommenterade en riktigt äcklig jävel
            ...
            put("blib_jackpot", JackpotTimerFloorItem.class);
    ...
    public static RoomItemFloor createFloor(long id, int baseId, Room room, int ownerId, String ownerName, int x, int y, double height, int rot, String data, LimitedEditionItemData limitedEditionItemData) {
        ...
        if (OrionSettings.storageItemQueueEnabled) {
        ...
        if (data.startsWith(GIFT_DATA)) {
            ...
            if (itemDefinitionMap.containsKey(def.getInteraction())) {
                try {
                    floorItem = itemDefinitionMap.get(def.getInteraction()).getConstructor(long.class, int.class, Room.class, int.class, String.class, int.class, int.class, double.class, int.class, String.class).newInstance(id, baseId, room, ownerId, ownerName, x, y, height, rot, data);
                } catch (Exception e) {
