package com.orionemu.server.game.rooms;
...
import com.orionemu.server.config.*;
import com.orionemu.server.game.players.types.Player;
import com.orionemu.server.game.rooms.filter.WordFilter;
import com.orionemu.server.game.rooms.models.CustomFloorMapData;
import com.orionemu.server.game.rooms.models.types.StaticRoomModel;
...
import com.orionemu.server.game.rooms.types.misc.ChatEmotionsManager;
...
public class RoomManager implements Initialisable {
    ...
    public static final int LRU_MAX_ENTRIES = Integer.parseInt(Configuration.currentConfig().getProperty("orion.game.rooms.data.max"));
    public static final int LRU_MAX_LOWER_WATERMARK = Integer.parseInt(Configuration.currentConfig().getProperty("orion.game.rooms.data.lowerWatermark"));
    ...
    public RoomData getRoomData(int id) {
        if (this.getRoomDataInstances().getMap().containsKey(id)) {
            return this.getRoomDataInstances().get(id).setLastReferenced(Orion.getTime());
        }
    ...
    public int createRoom(String name, String description, CustomFloorMapData model, int category, int maxVisitors, int tradeState, Session client, int wallTickness, int floorThickness, String decorations, boolean hideWalls) {
        ...
        return this.createRoom(name, model.getModelData(), client);
    }
