package com.orionemu.server.game.utilities.validator;
    ...
    public static void loadFigureData() {
        try {
            ...
            if (!figureDataFile.exists()) {
                furnidataDocument = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(Orion.class.getResourceAsStream("/config/figuredata.xml"));
                FileUtils.copyURLToFile(Orion.class.getResource("/config/figuredata.xml"), figureDataFile);
            } else {
            ...
            Orion.getServer().getLogger().warn("Error while initializing the PlayerFigureValidator", e);
        }
    }

    public static boolean isValidFigureCode(final String figureCode, final String genderCode) {
        if (!OrionSettings.playerFigureValidation) {
            return true;
        }
