...
public class KissCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        if (params.length != 1) {
            sendNotif(Locale.getOrDefault("command.kiss.none", "Vem vill du kyssa?"), client);
            return;
        }
        ...
        if (!((Math.abs((posX - playerX)) >= 2) || (Math.abs(posY - playerY) >= 2))) {
            client.getPlayer().getEntity().getRoom().getEntities().broadcastMessage(new WhisperMessageComposer(client.getPlayer().getEntity().getId(), "* " + client.getPlayer().getData().getUsername() + " " + Locale.getOrDefault("command.kiss.word", "kysser") + " " + kissedSession.getPlayer().getData().getUsername() + " *", 34));
