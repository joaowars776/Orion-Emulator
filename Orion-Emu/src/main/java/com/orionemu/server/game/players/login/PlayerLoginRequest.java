package com.orionemu.server.game.players.login;

import com.orionemu.api.events.players.OnPlayerLoginEvent;
...
import com.orionemu.server.network.messages.incoming.handshake.SSOTicketMessageEvent;
import com.orionemu.server.network.messages.outgoing.handshake.AuthenticationOKMessageComposer;
import com.orionemu.server.network.messages.outgoing.handshake.HomeRoomMessageComposer;
import com.orionemu.server.network.messages.outgoing.handshake.UniqueIDMessageComposer;
import com.orionemu.server.network.messages.outgoing.moderation.ModToolMessageComposer;
import com.orionemu.server.network.messages.outgoing.navigator.FavouriteRoomsMessageComposer;
...
import com.orionemu.server.network.messages.outgoing.user.details.AvailabilityStatusMessageComposer;
import com.orionemu.server.network.messages.outgoing.user.details.PlayerSettingsMessageComposer;
import com.orionemu.server.network.messages.outgoing.user.inventory.EffectsInventoryMessageComposer;
import com.orionemu.server.network.messages.outgoing.user.permissions.FuserightsMessageComposer;
import com.orionemu.server.network.sessions.Session;
import com.orionemu.server.network.sessions.SessionManager;
import com.orionemu.server.storage.queries.player.PlayerAccessDao;
...
public class PlayerLoginRequest implements OrionTask {
    ...
    public void run() {
        ...
        Player player = null;
        String message = " loggade precis in!";
        ...
        if (ipAddress != null && !ipAddress.isEmpty()) {
            ...
            if (PlayerManager.getInstance().getPlayerCountByIpAddress(ipAddress) > OrionSettings.maxConnectionsPerIpAddress) {
                client.disconnect();
                return;
            }
        }

        if (OrionSettings.saveLogins)
        ...
        NetworkManager.getInstance().getSessions().broadcast(new NotificationMessageComposer(client.getPlayer().getData().getFigure(), "" + client.getPlayer().getData().getUsername() + message + ""));
