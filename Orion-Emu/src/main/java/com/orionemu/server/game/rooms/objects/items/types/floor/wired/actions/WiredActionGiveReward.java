...
import com.orionemu.server.network.messages.outgoing.room.items.wired.WiredRewardMessageComposer;
...
public class WiredActionGiveReward extends WiredActionItem {
    ...
    public void onEventComplete(WiredItemEvent event) {
        ...
        if (OrionSettings.roomWiredRewardMinimumRank > this.ownerRank) return;
            ...
            switch (howOften) {
                ...
                case REWARD_LIMIT_DAY:
                    if (rewardTimings.get(this.getId()).containsKey(playerEntity.getPlayerId())) {
                        ...
                        if ((Orion.getTime() - lastReward) < ONE_DAY) {
                            errorCode = 2;
                        }
                    }
                    break;
                ...
                case REWARD_LIMIT_HOUR:
                    if (rewardTimings.get(this.getId()).containsKey(playerEntity.getPlayerId())) {
                        ...
                        if ((Orion.getTime() - lastReward) < ONE_HOUR) {
                            errorCode = 3;
                        }
                    }
                    break;
        ...
        if (rewardTimings.get(this.getId()).containsKey(playerEntity.getPlayerId())) {
            rewardTimings.get(this.getId()).replace(playerEntity.getPlayerId(), Orion.getTime());
        } else {
            rewardTimings.get(this.getId()).put(playerEntity.getPlayerId(), Orion.getTime());
        }
    }
