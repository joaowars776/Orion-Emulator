package com.orionemu.server.game.rooms.objects.entities.types;

import com.orionemu.api.game.rooms.entities.PlayerRoomEntity;
...
import com.orionemu.server.game.bots.BotType;
import com.orionemu.server.game.commands.CommandManager;
import com.orionemu.server.game.commands.vip.TransformCommand;
...
import com.orionemu.server.game.rooms.objects.entities.PlayerEntityAccess;
...
import com.orionemu.server.game.rooms.objects.entities.types.ai.bots.WaiterAI;
import com.orionemu.server.game.rooms.objects.items.RoomItemFloor;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.triggers.WiredTriggerPlayerSaysKeyword;
...
import com.orionemu.server.network.messages.outgoing.room.access.DoorbellRequestComposer;
import com.orionemu.server.network.messages.outgoing.room.access.RoomReadyMessageComposer;
import com.orionemu.server.network.messages.outgoing.room.alerts.CantConnectMessageComposer;
import com.orionemu.server.network.messages.outgoing.room.alerts.DoorbellNoAnswerComposer;
import com.orionemu.server.network.messages.outgoing.room.alerts.RoomErrorMessageComposer;
import com.orionemu.server.network.messages.outgoing.room.avatar.AvatarsMessageComposer;
import com.orionemu.server.network.messages.outgoing.room.avatar.IdleStatusMessageComposer;
...
import com.orionemu.server.network.messages.outgoing.room.engine.OpenConnectionMessageComposer;
import com.orionemu.server.network.messages.outgoing.room.engine.RoomForwardMessageComposer;
import com.orionemu.server.network.messages.outgoing.room.engine.RoomPropertyMessageComposer;
import com.orionemu.server.network.messages.outgoing.room.events.RoomPromotionMessageComposer;
...
import com.orionemu.server.network.messages.outgoing.room.permissions.YouAreOwnerMessageComposer;
import com.orionemu.server.network.messages.outgoing.room.permissions.YouAreSpectatorMessageComposer;
import com.orionemu.server.network.messages.outgoing.room.queue.RoomQueueStatusMessageComposer;
import com.orionemu.server.network.messages.outgoing.room.settings.RoomRatingMessageComposer;
...
public class PlayerEntity extends RoomEntity implements PlayerEntityAccess, Attributable, PlayerRoomEntity {
    ...
    public PlayerEntity(Player player, int identifier, Position startPosition, int startBodyRotation, int startHeadRotation, Room roomInstance) {
        ...
        if (LogManager.ENABLED)
            this.visitLogEntry = LogManager.getInstance().getStore().getRoomVisitContainer().put(player.getId(), roomInstance.getId(), Orion.getTime());
    }
    ...
    public boolean joinRoom(Room room, String password) {
        ...
        if (!isAuthFailed && !this.getPlayer().isBypassingRoomAuth() && (!isOwner && !this.getPlayer().getPermissions().getRank().roomEnterLocked() && !this.isDoorbellAnswered()) && !isTeleporting) {
            if (this.getRoom().getData().getAccess() == RoomAccessType.PASSWORD) {
                ...
                if (OrionSettings.roomEncryptPasswords) {
    ...
    public void leaveRoom(boolean isOffline, boolean isKick, boolean toHotelView) {
        ...
        if (this.visitLogEntry != null) {
            this.visitLogEntry.setExitTime((int) Orion.getTime());
