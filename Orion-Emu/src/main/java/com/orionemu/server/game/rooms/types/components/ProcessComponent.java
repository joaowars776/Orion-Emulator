...
import com.orionemu.server.game.rooms.objects.items.types.floor.TeleportPadFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.triggers.WiredTriggerWalksOffFurni;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.triggers.WiredTriggerWalksOnFurni;
...
public class ProcessComponent implements OrionTask {
    private Room room;
    ...
    public ProcessComponent(Room room) {
        ...
        this.adaptiveProcessTimes = OrionSettings.adaptiveEntityProcessDelay;
    }
...
//            log.debug("Room processing took " + (System.currentTimeMillis() - timeStart) + "ms");
        ...
        if (this.adaptiveProcessTimes) {
            OrionThreadManager.getInstance().executeSchedule(this, 500 - span.toMilliseconds(), TimeUnit.MILLISECONDS);
        }
    ...
    public void start() {
        ...
        if (this.adaptiveProcessTimes) {
            OrionThreadManager.getInstance().executeSchedule(this, 500, TimeUnit.MILLISECONDS);
        } else {
            this.processFuture = OrionThreadManager.getInstance().executePeriodic(this, 500, 500, TimeUnit.MILLISECONDS);
        }
    ...
    public void setDelay(int time) {
        this.processFuture.cancel(false);
        this.processFuture = OrionThreadManager.getInstance().executePeriodic(this, 0, time, TimeUnit.MILLISECONDS);
    }
