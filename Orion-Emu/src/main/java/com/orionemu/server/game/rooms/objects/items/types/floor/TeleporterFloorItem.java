...
import com.orionemu.server.game.rooms.objects.items.types.state.FloorItemEvent;
...
public class TeleporterFloorItem extends AdvancedFloorItem<TeleporterFloorItem.TeleporterItemEvent> {
    ...
    protected void onEventComplete(TeleporterItemEvent event) {
        try {
            ...
            Orion.getServer().getLogger().error("Failed to handle teleport event", e);
        }
    }
...
//
        ...
        try {
            ...
            Orion.getServer().getLogger().error("Failed to queue teleporter item event", e);
        }
    ...
    public void handleIncomingEntity(PlayerEntity entity, TeleporterFloorItem otherItem) {
        ...
        try {
            ...
            Orion.getServer().getLogger().error("Error while queueing teleport event", e);
        }
    }
