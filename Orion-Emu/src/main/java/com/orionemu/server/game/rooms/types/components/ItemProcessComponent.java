...
import com.orionemu.server.game.rooms.objects.items.queue.RoomItemEventQueue;
import com.orionemu.server.game.rooms.objects.items.types.floor.RollableFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.RollerFloorItem;
import com.orionemu.server.game.rooms.objects.items.types.floor.wired.triggers.WiredTriggerPeriodically;
...
public class ItemProcessComponent implements OrionTask {
    //    private final int INTERVAL = Integer.parseInt(Configuration.currentConfig().get("orion.system.item_process.interval"));
    ...
    public void start() {
        ...
        this.active = true;
        this.myFuture = OrionThreadManager.getInstance().executePeriodic(this, 0, INTERVAL, TimeUnit.MILLISECONDS);
...
//        }
        ...
        if (span.toMilliseconds() > FLAG && Orion.isDebugging) {
