package com.orionemu.server.storage.queries.navigator;
...
public class NavigatorDao {
    ...
    public static LinkedHashMap<Integer, FeaturedRoom> getFeaturedRooms() {
        Connection sqlConnection = null;
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;

        LinkedHashMap<Integer, FeaturedRoom> data = new LinkedHashMap<>();

        try {
            sqlConnection = SqlHelper.getConnection();

            preparedStatement = SqlHelper.prepare("SELECT * FROM navigator_featuredrooms WHERE enabled = 'true' ORDER BY id ASC", sqlConnection);

            resultSet = preparedStatement.executeQuery();

            while (resultSet.next()) {
                final int roomId = resultSet.getInt("room_id");
                data.put(roomId, new FeaturedRoom(resultSet));
            }
        } catch (SQLException e) {
            SqlHelper.handleSqlException(e);
        } finally {
