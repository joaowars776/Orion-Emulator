package com.orionemu.server.storage.queries.catalog;

import com.orionemu.server.boot.Orion;
import com.orionemu.server.game.catalog.CatalogManager;
import com.orionemu.server.game.catalog.types.CatalogFrontPageEntry;
        ...
        try {
            ...
            while (resultSet.next()) {
                try {
                    ...
                    Orion.getServer().getLogger().warn("Failed to load catalog page: " + resultSet.getInt("id"));
                }
            }
            ...
            while (resultSet.next()) {
                try {
                    ...
                    if (!catalogItem.getItemId().equals("-1") && catalogItem.getItems().size() == 0) {
                        Orion.getServer().getLogger().warn(String.format("Catalog Item with ID: %s and name: %s has invalid item data! (Data: %s)", catalogItem.getId(), catalogItem.getDisplayName(), catalogItem.getItemId()));
                        continue;
                    }
                    ...
                    exception.printStackTrace();
                    Orion.getServer().getLogger().warn("Failed to load catalog item: " + resultSet.getString("id"));
                }
            }
        } catch (SQLException e) {
            SqlHelper.handleSqlException(e);
...
//                }
                ...
                try {
                    ...
                    Orion.getServer().getLogger().warn("Error while loading catalog item: " + resultSet.getInt("id"));
                }
            }
