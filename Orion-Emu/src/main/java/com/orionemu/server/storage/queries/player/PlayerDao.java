...
public class PlayerDao {
    public static Player getPlayer(String ssoTicket) {
        ...
        try {
            ...
            preparedStatement = SqlHelper.prepare(
                    " SELECT p.id as playerId, p.username AS playerData_username, p.snow_xp AS playerData_snow, p.figure AS playerData_figure, p.motto AS playerData_motto, p.credits AS playerData_credits, p.vip_points AS playerData_vipPoints, p.rank AS playerData_rank, p.last_ip AS playerData_lastIp," +
    ...
    public static PlayerData getDataByUsername(String username) {
        ...
        try {
            ...
            preparedStatement = SqlHelper.prepare("SELECT id, motto, figure, gender, email, rank, credits, vip_points, activity_points, reg_date, last_online, vip, achievement_points, reg_timestamp, favourite_group, last_ip, quest_id, time_muted, snow_xp FROM players WHERE username = ?", sqlConnection);
            ...
            while (resultSet.next()) {
                return new PlayerData(resultSet.getInt("id"), username, resultSet.getString("motto"), resultSet.getString("figure"), resultSet.getString("gender"),
                        ...
                        resultSet.getInt("achievement_points"), resultSet.getInt("reg_timestamp"), resultSet.getInt("favourite_group"), resultSet.getString("last_ip"), resultSet.getInt("quest_id"), resultSet.getInt("time_muted"), resultSet.getBoolean("nux"), 0, resultSet.getInt("snow_xp"));
            }
        } catch (SQLException e) {
            SqlHelper.handleSqlException(e);
        } finally {
    ...
    public static PlayerData getDataById(int id) {
        ...
        try {
            ...
            preparedStatement = SqlHelper.prepare("SELECT id, username, motto, figure, gender, email, rank, credits, vip_points, activity_points, reg_date, last_online, vip, achievement_points, reg_timestamp, favourite_group, last_ip, quest_id, time_muted, snow_xp FROM players WHERE id = ?", sqlConnection);
    ...
    public static void updatePlayerStatus(Player player, boolean online, boolean setLastOnline) {
        ...
        try {
            ...
            if (setLastOnline) {
                preparedStatement.setLong(2, Orion.getTime());
    ...
    public static String generateFastFoodTicket(int playerId) {
        Connection sqlConnection = null;
        PreparedStatement preparedStatement = null;

        String token = "FASTFOOD-" + playerId + "-ASTRO";

        try {
            sqlConnection = SqlHelper.getConnection();

            preparedStatement = SqlHelper.prepare("UPDATE players SET fastfood_token = ? WHERE player_id = ?", sqlConnection);
